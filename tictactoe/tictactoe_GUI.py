...
class tictactoeGUI:
    ...
    def init_GUI(self):
        ...
        self.root.title('틱텍토')
        self.root.geometry(f'{self.CANVAS_SIZE}x{self.CANVAS_SIZE}') #창 크기는 문자로 넣음
        self.root.geometry('300x300') #제목을 뺀 나머지 크기
        self.root.resizable(width=False, height=False) #창 사이즈 고정
        ...
        self.canvas.bind('<Button-1>', self.click_handler) #버튼을 <Button-1>로 연결, 눌리면 click_handler 함수실행
                                                            #***** click_handler 괄호 넣지 않음 ****



        self.root.mainloop()    #tkinter 실행
    ...
    def draw_board(self):
        ...
        for i,v in enumerate(self.game_engine.board):
            if v == '.':
                ...
                self.canvas.create_image(x,y, anchor='nw', image=self.images[v]) #북서쪽을 기준으로 이미지를 넣는다
            x += TILE_SIZE
